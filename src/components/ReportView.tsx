...
export const ReportView: React.FC<ReportViewProps> = ({ data, recordedVideo, recordedRawBlob, synthesizing = false, onReset }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-950 z-50 overflow-y-auto px-6 py-12 text-slate-200 font-sans">
      ...
      >
        ...
        {/* ========= 录制回放区域 ========= */}
        {synthesizing && (
          <div className="mb-8 flex items-center justify-center py-16 rounded-[40px] bg-slate-900 border border-slate-800">
            <div className="text-center space-y-3">
              <svg className="w-8 h-8 text-orange-500 animate-spin mx-auto" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
              </svg>
              <p className="text-sm font-bold text-slate-200">正在合成高清标注视频…</p>
              <p className="text-[10px] text-slate-500">1280×720 · H.264 高码率</p>
            </div>
          </div>
        )}
        {recordedVideo && (
